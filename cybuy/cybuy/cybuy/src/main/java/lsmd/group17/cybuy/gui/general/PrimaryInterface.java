...
public class PrimaryInterface extends GraphicInterface {
    ...
    public void initialize(){
        ...
        firstPageButton.setOnMouseClicked(event -> goToSelectedPage(event, 1));
        secondPageButton.setOnMouseClicked(event -> goToSelectedPage(event, 2));
        thirdPageButton.setOnMouseClicked(event -> goToSelectedPage(event, 3));
    ...
    private void selectSubCategory(MouseEvent event, Label subcategory) {
        unsetBoldness(videogames_subcategories);
        unsetBoldness(categories);
        searchBar.setText("");
        deletePreviousChanges();
    ...
    private void showSubsection(Label category){
        if(!show_subsection){
            ...
            for(Label subcategory : videogames_subcategories) {
                subcategory.setOnMouseClicked(event -> selectSubCategory(event, subcategory));
            }
    ...
    private void initializeTotalPages() {
        ...
        if(totalPages <= 1) next_page = false;
        else next_page = true;
    ...
    private void goToSelectedPage(MouseEvent event, int position){
    ...
    public Label getRegisterLabel(){ return register; }
    public Label getLoginLabel(){ return login;}
    public AnchorPane getContainerBox() { return containerBox; }

    public StackPane getCart_btn() {
        return cart_btn;
    }
}

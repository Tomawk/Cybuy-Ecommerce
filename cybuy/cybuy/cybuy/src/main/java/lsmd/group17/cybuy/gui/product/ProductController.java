...
public class ProductController extends Controller {
        ...
        if(user != null){
            if(relativeProduct != null && user.getUsername().equals(relativeProduct.getSellerUsername())) {
                ...
                removeProduct.setOnMouseClicked((event) -> {
                    //TODO dire se la rimozione c'è stata
            ...
            if(user.getRole().equals("User")) {
                ...
                addWishlist.setOnMouseClicked((event) -> {
                    //TODO dire se l'inserimento c'è stato o no
                    boolean inserted = EventHandler.getInstance().addProductToWishlist(session.getUserLogged(), relativeProduct);
                });
                addWishlist.setOnMouseClicked((event) -> EventHandler.getInstance().addProductToWishlist(session.getUserLogged(), relativeProduct));
            }else{
    ...
    private void applyChanges(Event e){
        ...
        try {
            ...
            //TODO do something if response is false
            if(!response)
                System.err.println("ERROR it is impossible to modify the current product, retry");

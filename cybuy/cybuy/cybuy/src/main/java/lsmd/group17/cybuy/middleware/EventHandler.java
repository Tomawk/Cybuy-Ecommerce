...
public class EventHandler {
    ...
    private EventHandler() {
        ConfigurationParameters config = null;
        try {
            config = Utilities.readParametersFromXML("cybuy/config.xml");
    ...
    public User registrationFormSignUpAction(ArrayList<String> inputStrings,
        ...
        if (inputStrings.get(2).equals(inputStrings.get(3))) {
            ...
            User new_user = User.UserBuilder()
                    ...
                    .setPassword(inputStrings.get(2))
    ...
    public ArrayList<User> getSellersRanking(String username, int sort) {
        //TODO sistemare la analytics di questa parte
     ...
     * @return  true if the deletion was successful
     */
    public boolean deleteUser(User user){ //TODO TESTARE

        boolean user_deleted = mongoDBDriver.deleteUser(user);
        if(user_deleted) {
            boolean flushed = false;
            flushed = flushCart(user);
            flushed = flushWishlist(user);
    ...
    public void createAndInsertAnalytics(User user) {
        String seller_username = user.getUsername();
        mongoDBDriver.getReviewsAnalytics(user);
        int total_reviews = user.getTotalReviews();
        double avg_reviews = user.getAverageReview();
        int [] star_distribution = mongoDBDriver.getNumberOfStars(user);
        int avg_delivery = mongoDBDriver.getAverageDeliveryTime(user);
        double total_earnings = mongoDBDriver.getTotalEarnings(user);
        int total_sales = mongoDBDriver.getTotalSales(user);
        Product bestSellingProduct = mongoDBDriver.getBestSellingProduct(user);

        Analytics new_analytics = Analytics.AnalyticsBuilder()
                .setSellerUsername(seller_username)
                .setTotalReviews(total_reviews)
                .setAvgReviews(avg_reviews)
                .setStarsDistribution(star_distribution)
                ...
                .setMonth(2).build(); //TODO rivedi

        mongoDBDriver.insertAnalyticsCollection(new_analytics);

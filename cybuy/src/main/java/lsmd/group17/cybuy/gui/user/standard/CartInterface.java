...
public class CartInterface extends GraphicInterface {
    ...
    private final Label total_amount, temporary_amount;
    ...
    private void CheckoutAction(MouseEvent event){
        if(eventHandler.placeOrder(session.getUserLogged(), productArrayList)) {
            if(!eventHandler.flushCart(session.getUserLogged())) {
                logMessage("Order placed successfully. Error emptying the cart.", true);
            }
            logMessage("Order placed successfully!", false);
        } else {
            logMessage("Order was not placed successfully. Please try again or remove products from your cart.", true);
        }
        setCart();
        getTemporaryAmount();
        getTotalAmount();
    ...
    private void logMessage(String message, boolean error) {
        StackPane containerMsg = new StackPane();
        Text messageText = new Text(message);
        messageText.setTextAlignment(TextAlignment.CENTER);
        containerMsg.getChildren().add(messageText);
        containerMsg.setPrefWidth(1200); containerMsg.setPrefHeight(25);
        containerMsg.setLayoutX(0); containerMsg.setLayoutY(50);
        if(error) {
            containerMsg.setStyle("    -fx-background-color: linear-gradient(to top, #770000, #ff000);" +
                    "-fx-font-weight: bold;");
        } else {
            containerMsg.setStyle("-fx-background-color: linear-gradient(to top, #00ff10, #90ff10); " +
                    "-fx-font-weight: bold;");
        }
